...
/** Pas d'endpoint "mes missions" cote backend (livrable H) : filtre client par role, comme pour les publications. */
...
export function useMission(id: number | undefined) {
  ...
  return data?.find((mission) => mission.id === id);
}

/** /topic/missions est deja emis par le backend a chaque creation/annulation/fin de mission — jamais ecoute par l'ancien frontend. */
